...
#         print("pred_class",pred_class)
        ...
        total_logits.append(pred_class)
        total_labels.append(targets)       
    # gather the stats from all processes
    total_logits=torch.cat(total_logits,dim=0)
    total_labels=torch.cat(total_labels,dim=0)
    auc=roc_auc_score(total_logits.cpu(),total_labels.cpu())
    #print (total_logits.shape,total_labels.shape)
    metric_logger.synchronize_between_processes()
    #print("auc",auc)
    print("Averaged stats:", metric_logger.global_avg(),"auc",auc)   
    return {k: "{:.4f}".format(meter.global_avg) for k, meter in metric_logger.meters.items()}
        ...
        if not args.evaluate:
            ...
            for key in list(state_dict.keys()):                
                if 'bert' in key:
                    ...
                    del state_dict[key]
#         state_dict['text_encoder.embeddings.word_embeddings.weight'].resize_(21128, 768)
#         state_dict['text_encoder_m.embeddings.word_embeddings.weight'].resize_(21128, 768)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--checkpoint', default='/root/autodl-tmp/TCL_4M.pth')
