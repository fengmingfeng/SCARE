...
def train(model, data_loader, optimizer, tokenizer, epoch, warmup_steps, device, scheduler, config):
    ...
    for i, (image, image_aug, text) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        # print("image.shape", image.shape) image.shape torch.Size([8, 3, 256, 256])
        image_aug = image_aug.to(device,non_blocking=True)
        # print("image_aug", image_aug.shape)
...
def main(args, config):
    ...
    tokenizer = BertTokenizer.from_pretrained("/root/autodl-tmp/ETCL/bert/")
    #### Model #### 
