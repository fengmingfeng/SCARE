...
class ALBEF(nn.Module):
    ...
    def forward(self, image, image_aug, text, alpha=0):
        ...
        # print("image_embeds",image_embeds.shape) image_embeds torch.Size([8, 257, 768])
        image_atts = torch.ones(image_embeds.size()[:-1],dtype=torch.long).to(image.device)
        # print("image_atts",image_atts.shape) [8,257]
        # print("image_embeds[:,0,:]", image_embeds[:, 0, :].shape) [8,768]   #8个CLS,每个768维
        image_feat = F.normalize(self.vision_proj(image_embeds[:,0,:]),dim=-1)
        # print("image_feat",image_feat.shape) [8,256]
        text_output = self.text_encoder.bert(text.input_ids, attention_mask = text.attention_mask,                      
                                        return_dict = True, mode = 'text')            
        text_embeds = text_output.last_hidden_state
        # print("text_embeds", text_embeds.shape) [8,x,768]
        text_feat = F.normalize(self.text_proj(text_embeds[:,0,:]),dim=-1)
        # print("text_feat", text_feat.shape)  [8,256]
        # get momentum features
        with torch.no_grad():
            self._momentum_update()
            image_embeds_m = self.visual_encoder_m(image_aug)
            # print("image_embeds_m", image_embeds_m.shape)  torch.Size([8, 257, 768])
            image_feat_m = F.normalize(self.vision_proj_m(image_embeds_m[:,0,:]),dim=-1)
            # print("image_feat_m", image_feat_m.shape)   [8,256]
            # jinyu: local features of visual part
            # print("image_embeds_m[:,1:,:]", image_embeds_m[:,1:,:].shape) [8,256,768] 出去CLS部分，原来为[8,257,768]
            image_feat_m_l = F.normalize(self.vision_proj_m(image_embeds_m[:,1:,:]),dim=-1)
            # print("image_feat_m_l", image_feat_m_l.shape) [8,x,256]
            image_feat_m_l = self.patch_pooling(image_feat_m_l) # pooling for image patches
            # print("image_feat_m_l", image_feat_m_l.shape) [8,16,256] [256,256]的图像分为[16,16]的patch
            # print(image_feat_m.t().shape) [256,8] t()的转置 65536+8
            image_feat_all = torch.cat([image_feat_m.t(),self.image_queue.clone().detach()],dim=1)
            # print("image_feat_all", image_feat_all.shape) [256,65544]
            text_output_m = self.text_encoder_m.bert(text.input_ids, attention_mask = text.attention_mask,                      
                                                return_dict = True, mode = 'text')    
            text_feat_m = F.normalize(self.text_proj_m(text_output_m.last_hidden_state[:,0,:]),dim=-1)
            # print("text_feat_m",text_feat_m.shape) [8,256]
            # jinyu: local features of text part
            text_feat_m_l = F.normalize(self.text_proj_m(text_output_m.last_hidden_state[:,1:,:]),dim=-1)
            # print("text_feat_m_l", text_feat_m_l.shape) [8,x,256]
            text_feat_all = torch.cat([text_feat_m.t(),self.text_queue.clone().detach()],dim=1)
            # print("text_feat_all", text_feat_all.shape)  [256,65544]
            sim_i2t_m = image_feat_m @ text_feat_all / self.temp  #[8,65544]
            sim_t2i_m = text_feat_m @ image_feat_all / self.temp     

            sim_targets = torch.zeros(sim_i2t_m.size()).to(image.device)
            # print("sim_targets", sim_targets.shape) [8,65544]
    ...
    def patch_pooling(self, x):
        ...
        # print("x",x.shape) [8,16,16,256]
        x = x.permute(0,3,1,2)
        # print("x", x.shape)  torch.Size([8, 256, 16, 16])
        c1 = int(np.sqrt(b1))
        x = F.avg_pool2d(x, c1, stride=c1)
        # print("x", x.shape) torch.Size([8, 256, 4, 4])
        x = x.permute(0,2,3,1).reshape(batch_size, c1*c1, dim)
        # print("x", x.shape) [8,16,256]
        return x
