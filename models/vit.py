...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
        ...
        self.num_tokens=10
        #随机初始化
        val = math.sqrt(6. / float(3 * reduce(mul, patchSize, 1) + prompt_dim))
        a = reduce(mul,patchSize,1)
        b = 3*a
        c = float(b+prompt_dim)
...
# #         print(aaaaaaaaaaaaaaaaaaaaaa)
        for i,blk in enumerate(self.blocks):
            if i == 0 :
                hidden_states = blk(x, register_blk==i)
#             print(blk)
            else:
                if i <= self.deep_prompt_embeddings.shape[0]:
                    deep_prompt_emb = self.prompt_dropout(self.prompt_proj(
                        self.deep_prompt_embeddings[i-1]).expand(B, -1, -1))
                    # print("self.deep_prompt_embeddings[i-1]", self.deep_prompt_embeddings[i-1].shape) [10,768]
                    # print("deep_prompt_emb.shape", deep_prompt_emb.shape) [128,10,768]
                    # print("hidden_states[:, :1, :].shape", hidden_states[:, :1, :].shape) [128,1,768]
                    # print("deep_prompt_emb.shape", deep_prompt_emb.shape) [128,10,768]
                    # print("hidden_states[:, (1+self.num_tokens):, :].shape", hidden_states[:, (1+self.num_tokens):, :].shape) [128,196,768]
