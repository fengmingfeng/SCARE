...
class ALBEF(nn.Module):
    def __init__(self,                 
        ...
        self.distill = config['distill'] #distill为true

        self.visual_encoder = VisionTransformer(
            img_size=config['image_res'], patch_size=16, embed_dim=768, depth=12, num_heads=12, 
            mlp_ratio=4, qkv_bias=True, norm_layer=partial(nn.LayerNorm, eps=1e-6))    #partial用于固定部分参数，之后用norm_layer调用，nn.LayerNorm类似于原函数，eps为固定的参数，用于防止除0的最小值
    ...
    def forward(self, image, quesiton, answer=None, alpha=0, k=None, weights=None, train=True):
        ...
        #         print("image_embeds", image_embeds.shape)   image_embeds torch.Size([8, 577, 768])
        #         print(image_embeds.size()[:-1])[8,577]
        image_atts = torch.ones(image_embeds.size()[:-1],dtype=torch.long).to(image.device)
        #         print(image_atts)[8,577]全为1的张量
        # print("image_atts.shape", images_atts.shape)    image_atts.shape torch.Size([8, 577])
        if train:               
            '''
            k: number of answers for each question
            weights: weight for each answer
            '''
            #将answer.input_ids中的0变为-100，并赋值给answer_targets
            answer_targets = answer.input_ids.masked_fill(answer.input_ids == self.tokenizer.pad_token_id, -100)      
            #attention_mask可选
            #encoder_hidden_states可选，encoder最后一层的隐含状态序列，模型配置为decoder时需要此输入
            #encoder_attention_mask可选，encoder最后一层的隐含状态序列是否参与attention计算,防止padding部分参与，模型配置为decoder时需要此输入
            ...
            for b, n in enumerate(k):
                #                 print("question_output.last_hidden_state[",b,"]",question_output.last_hidden_state[b].shape)
                #                 print("quesiton.attention_mask[",b,"]",quesiton.attention_mask[b].shape)
                question_states += [question_output.last_hidden_state[b]]*n
                question_atts += [quesiton.attention_mask[b]]*n
                #             print(len(question_states))
                #             print(question_states)
                #             print(type(question_states))
                #             print(type(question_atts))
            question_states = torch.stack(question_states,0)    
            question_atts = torch.stack(question_atts,0)     
            #torch.stack()按某一维度展开，扩展了一个维度;这里按第一维展开
            if self.distill:                    
                with torch.no_grad():
    ...
    def _momentum_update(self):
        for model_pair in self.model_pairs:           
            for param, param_m in zip(model_pair[0].parameters(), model_pair[1].parameters()):
               ...
               # m*(第一个模型参数)+(1-m)*第二个模型参数，如结构图中的动量编码器所示
